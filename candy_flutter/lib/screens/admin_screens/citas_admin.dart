...
import '../compartidas/citas_detalles.dart';

class VerCitasPage extends StatefulWidget {
  const VerCitasPage({super.key});

  @override
  State<VerCitasPage> createState() => _VerCitasPageState();
}

class _VerCitasPageState extends State<VerCitasPage> {
  final CitasApi _citasApi = CitasApi();
  ...
  Future<void> _cargarCitas() async {
    try {
      final citas = await _citasApi.obtenerCitas();
      setState(() {
        _citas = citas;
  ...
  void _filtrarCitas() {
    setState(() {
      _citasFiltradas =
          _citas.where((cita) {
            final cliente = cita['cliente_nombre']?.toLowerCase() ?? '';
            final manicurista = cita['manicurista_nombre']?.toLowerCase() ?? '';
            final fecha = cita['Fecha'];

            final coincideCliente = cliente.contains(
              _busquedaCliente.toLowerCase(),
            );
            final coincideManicurista = manicurista.contains(
            ...
            return coincideCliente && coincideManicurista && coincideFecha;
          }).toList();
    });
  }
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return Padding(
                                  ...
                                  child: Card(
                                    margin:
                                        EdgeInsets
                                            .zero, // El padding ya está en el Padding exterior
                                    elevation: 4,
                                    shape: RoundedRectangleBorder(
